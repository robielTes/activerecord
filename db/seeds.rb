...
100.times do |id|
  Order.create!(created_at: Faker::Time.between(from: DateTime.now - 365, to: DateTime.now), shipped_at: Faker::Time.between(from: DateTime.now - 365, to: DateTime.now),status: Faker::Subscription.status, clients_id: Client.random_id)
end
puts "Created #{Order.count} orders"

200.times do |id|
  OrderItem.create!(quantity: Faker::Number.between(from: 1, to: 2400), item_price: Faker::Commerce.price(range: 50..100.0), products_id: Product.random_id, orders_id: Order.random_id)
end
